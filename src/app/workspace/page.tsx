...
export default function Workspace() {
  ...
  const getStatusColor = (status: Job["status"]) => {
    switch (status) {
      ...
      case "compiling": return "text-orange-400 bg-orange-950/40 border-orange-900/60 animate-pulse";
      case "researching": return "text-amber-400 bg-amber-950/40 border-amber-900/60";
      default: return "text-zinc-400 bg-zinc-900/60 border-zinc-800/80";
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row min-h-screen text-zinc-100 font-sans relative bg-black">
      
      {/* Decorative Orbs */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bronze-glow-orb pointer-events-none -z-10 opacity-40" />
      <div className="absolute bottom-1/3 right-1/4 w-96 h-96 copper-glow-orb pointer-events-none -z-10 opacity-40" />
      <div className="dot-grid" />

      {/* Sidebar: Lists past generations */}
      <aside className="w-full md:w-80 glass-panel border-r border-zinc-900/60 flex flex-col shrink-0 md:sticky md:top-0 md:h-screen z-10 bg-zinc-950/60">
        ...
        <div className="p-5 border-b border-zinc-900 flex flex-col space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              <Link
                href="/"
                className="p-2 rounded-lg bg-zinc-900/60 border border-zinc-800/80 hover:bg-zinc-800/80 hover:border-zinc-600/80 text-zinc-400 hover:text-zinc-100 transition-all cursor-pointer"
          ...
          <div className="grid grid-cols-2 gap-2 text-[9px] font-mono text-zinc-500 bg-zinc-950 p-2 rounded-lg border border-zinc-900/60">
            <div className="flex justify-between">
              <span>SYS.QUEUE:</span>
              <span className="text-emerald-400 font-bold">OK</span>
            </div>
            <div className="flex justify-between border-l border-zinc-900 pl-2">
              <span>VOXELS:</span>
              <span className="text-amber-500 font-bold">5.0M</span>
            </div>
          </div>
        </div>
        ...
        {/* Jobs list */}
        <div className="flex-grow overflow-y-auto divide-y divide-zinc-900/40 p-3 space-y-2">
          {jobs.length === 0 ? (
            <div className="py-8 text-center text-zinc-500 text-xs font-mono border border-dashed border-zinc-900 rounded-xl bg-zinc-950/20">
              ...
              <div
                ...
                className={`group w-full text-left p-3.5 rounded-xl border transition-all cursor-pointer relative ${
                  selectedJob?.jobId === job.jobId
                    ? "bg-zinc-900/50 border-amber-500/40 shadow-[0_0_15px_rgba(245,158,11,0.08)]"
                    : "bg-transparent border-transparent hover:bg-zinc-900/20 hover:border-zinc-900/60"
                }`}
              >
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <div className="flex items-center space-x-1.5">
                    ...
                    <button
                      ...
                      className="p-1 rounded text-zinc-500 hover:text-red-400 hover:bg-red-950/30 transition-all cursor-pointer"
                      title="Delete Run"
                    >
                ...
                {job.componentType && (
                  <div className="mt-2.5 flex items-center space-x-2 text-[9px] font-mono text-zinc-400">
                    <span className="bg-zinc-950/80 px-2 py-0.5 rounded border border-zinc-900/80 text-amber-500/80 uppercase tracking-wider font-bold">
      ...
      {/* Main Panel */}
      <main className="flex-grow flex flex-col bg-zinc-950/10 min-h-screen z-10">
        ...
        {isCreating ? (
          <div className="max-w-3xl mx-auto w-full p-6 md:py-16 space-y-8 animate-fade-in-up">
            ...
            <div className="text-center space-y-4">
              ...
              <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-amber-250 to-orange-500 font-black">
            ...
            <form onSubmit={handleSubmit} className="glass-panel rounded-2xl p-6 md:p-8 glow-card space-y-6">
              ...
              <div className="space-y-2.5">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-zinc-950/80 border border-zinc-900/60 rounded-xl p-4 text-sm font-sans focus:outline-none focus:border-amber-500/80 focus:ring-1 focus:ring-amber-500/30 transition-all leading-relaxed placeholder-zinc-700 text-zinc-100"
                />
              </div>
                ...
                >
                  ...
                  {file ? (
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-amber-500">{file.name}</p>
              ...
              >
                {isSubmitting ? (
                  <>
                    <Cpu className="h-4 w-4 animate-spin text-zinc-950" />
          ...
          selectedJob && (
            <div key={selectedJob.jobId} className="p-6 md:p-8 space-y-6 flex-grow flex flex-col max-w-6xl mx-auto w-full animate-fade-in-up">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-900 pb-5 gap-4">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2 text-[10px] font-mono text-zinc-500">
                    <span className="bg-zinc-900 px-2 py-0.5 rounded border border-zinc-800 text-zinc-400">JOB RUN</span>
                    <span>ID:</span>
                    <span className="text-amber-500 font-bold font-mono">{selectedJob.jobId}</span>
                  </div>
                  <h2 className="text-lg md:text-xl font-bold font-sans tracking-tight text-zinc-100">
                    {selectedJob.prompt}
                  </h2>
                </div>
              ...
              <div className="glass-panel rounded-2xl p-5 border border-zinc-900/60 bg-zinc-950/40">
                <div className="flex justify-between text-[11px] font-mono text-zinc-500 mb-2.5">
                  <span className="tracking-wider">SYSTEM PIPELINE TRACKER</span>
                  <span className="text-amber-500 font-bold">{selectedJob.progress}%</span>
                </div>
                <div className="w-full bg-zinc-950 border border-zinc-900/60 h-2.5 rounded-full overflow-hidden mb-5">
                  <div
                    className="h-full bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 transition-all duration-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-1">
                  <div className="flex items-center space-x-2.5">
                    <div className={`h-3 w-3 rounded-full transition-all ${
                      selectedJob.progress >= 10 ? "bg-amber-500 step-glow-bronze" : "bg-zinc-900 border border-zinc-800"
                    }`} />
                    <span className="text-xs font-mono text-zinc-400">1. Ingest Prompt</span>
                  </div>
                  <div className="flex items-center space-x-2.5">
                    <div className={`h-3 w-3 rounded-full transition-all ${
                      selectedJob.progress >= 40 ? "bg-amber-500 step-glow-bronze" : "bg-zinc-900 border border-zinc-800"
                    }`} />
                    <span className="text-xs font-mono text-zinc-400">2. Research Agent</span>
                  </div>
                  <div className="flex items-center space-x-2.5">
                    <div className={`h-3 w-3 rounded-full transition-all ${
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                ...
                <div className="glass-panel rounded-xl overflow-hidden border border-zinc-900/60 flex flex-col h-[420px] shadow-2xl relative">
                  ...
                  <div className="bg-zinc-950/80 border-b border-zinc-900 px-4 py-3 flex items-center justify-between shrink-0">
                    <div className="flex items-center space-x-2">
                      <TerminalIcon className="h-4 w-4 text-amber-500" />
                      <span className="text-[10px] font-mono font-bold tracking-widest text-zinc-500 uppercase tracking-widest">
                        COMPILER LOG CONSOLE
                      </span>
                    </div>
                    {/* Glowing status */}
                    <div className="flex items-center space-x-2.5 bg-zinc-900/60 px-2.5 py-1 rounded-md border border-zinc-800/60">
                      <span className="h-2 w-2 rounded-full bg-amber-500 animate-ping" />
                      <span className="text-[9px] font-mono font-bold text-zinc-400 uppercase tracking-wider">{selectedJob.status}</span>
                    </div>
                  </div>
                ...
                <div className="glass-panel rounded-2xl overflow-hidden h-[420px] shadow-2xl relative cyber-scanline">
                  ...
                  {selectedJob.status === "completed" && selectedJob.finalDimensions ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-zinc-500 text-center p-6 space-y-4 bg-zinc-950/60">
                      ...
                      <div className="space-y-1">
                        <h4 className="font-mono text-sm text-amber-500 uppercase tracking-wider font-bold">Compiling 3D Geometry</h4>
                        <p className="text-xs max-w-xs leading-relaxed text-zinc-400">
              ...
              {selectedJob.status === "completed" && selectedJob.originalDimensions && selectedJob.finalDimensions && (
                <div className="glass-panel rounded-2xl p-6 space-y-6 border border-zinc-900/60 shadow-xl bg-zinc-950/40">
                  
                  {/* Title & Metadata */}
                  <div className="flex items-center justify-between border-b border-zinc-900/60 pb-4">
                    <h3 className="font-mono text-xs font-bold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-orange-600 flex items-center space-x-2 uppercase">
                          ...
                          <div
                            ...
                            className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${
                              modified
                                ...
                                : "bg-zinc-900/25 border-zinc-900/80 hover:border-zinc-800"
                            }`}
                          >
                            <div className="flex justify-between items-start">
                              <span className="block text-[9px] font-mono text-zinc-400 uppercase tracking-widest font-bold">
                                {formatKeyName(key)}
                              </span>
                              {!modified ? (
                            ...
                            <div className="w-full bg-zinc-950/80 h-1 rounded-full overflow-hidden mt-1">
                              <div 
                                className={`h-full rounded-full ${modified ? "bg-orange-500" : "bg-amber-500"}`}
              ...
              {(selectedJob.status === "completed" || selectedJob.status === "failed") && (
                <form onSubmit={(e) => handleModifySubmit(e, false)} className="glass-panel rounded-2xl p-5 glow-card space-y-4 border border-zinc-900/60 bg-zinc-950/40 shadow-xl">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-mono font-bold text-zinc-400 bg-zinc-950 border border-zinc-900/60 px-2.5 py-0.5 rounded-md tracking-wider">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <input
                      ...
                      className="flex-grow bg-zinc-950/80 border border-zinc-900/60 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-amber-500/80 focus:ring-1 focus:ring-amber-500/30 transition-all font-sans text-zinc-100 placeholder-zinc-700"
                    />
                    <div className="flex gap-2.5">
                      <button
                        ...
                        className="bg-zinc-900/60 border border-zinc-850 hover:border-zinc-700 hover:bg-zinc-850 text-zinc-200 font-mono font-bold text-xs py-3.5 px-5 rounded-xl flex items-center space-x-2 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
